...
func (s *suite) TestHooksExecution(t *testing.T, query string, args ...interface{}) {
	...
	t.Run("Query", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, 1, before, "Before Hook did not run for query: "+query)
		assert.Equal(t, 1, after, "After Hook did not run for query:  "+query)
	})
	...
	t.Run("Statements", func(t *testing.T) {
		...
		assert.Equal(t, 0, before, "Before Hook run before execution: "+query)
		assert.Equal(t, 0, after, "After Hook run before execution:  "+query)
